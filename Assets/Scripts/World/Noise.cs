...
public class Noise  {
    ...
    public static float[,,] GenerateNoiseMap(int seed, int chunkSize, int chunkHeight, Vector3 noiseOffset, float noiseScale, float noiseWeight, int octaves, float persistance, float lacunarity) {
        ...
        float maxPossibleNoise = 0; //estimate the range of values
        float amp = 1;
        float freq = 1;
        ...
        float maxPossibleHeight = chunkHeight + maxPossibleNoise * noiseWeight;
        ...
        for (int z = 0; z < chunkSize + 1; z++) {
            for (int y = 0; y < chunkHeight + 1; y++) {
                for (int x = 0; x < chunkSize + 1; x++) {
                    ...
                    noiseMap[z, y, x] = Mathf.InverseLerp(0, maxPossibleHeight, noiseMap[z, y, x]);
                }
            }
        }
